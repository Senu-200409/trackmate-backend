                ...
                if (res.ResultStatusCode == "1")
                {
                    ...
                    foreach (DataRow row in res.ResultDataTable.Rows)
                    {
                        BusDetailsModel Bus = new BusDetailsModel
                        {
                            ...
                            Vehicle = row["tbd_vehicle"].ToString(),
                            SheetCount = row["tbd_sheet_count"].ToString(),
                            LicenseExpiry = row["tbd_license_expiry"].ToString(),
                            InsuranceExpiry = row["tbd_insurance_expiry"].ToString(),
                            Latitude = row["tbd_location_latitude"].ToString(),
                            Longitude = row["tbd_location_longitude"].ToString(),
                           // SchoolID = row["tbd_school_id"].ToString(),
                            CurrentStatus = row["tbd_current_status"].ToString(),
